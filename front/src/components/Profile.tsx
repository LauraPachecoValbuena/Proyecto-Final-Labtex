...
const EditMyUser: React.FC<IPropsGlobal & RouteComponentProps<{ user_id: string }>> = (props) => {
			...
			fetch('http://localhost:3000/api/users/edit/' + user._id, {
				...
				if (response.ok) {
					response.json().then((u) => {
						...
						props.history.push('/users/list'); //aquí tiene q ser otra RUTA.
					});
				}
			});
		} else {
			...
			<div className="row justify-content-center">
				<div className="col-6">
					<div className="form-group" id="formEdit">
						...
						<button type="submit" className="btn btn-outline-info" onClick={() => Edit(user._id)}>
							Save
						</button>
